...
for phase in np.arange(-360/config.lambda_over_d_spacing, 360/config.lambda_over_d_spacing, 2): # sweep over angle
    ...
    arrays_adjusted = adjust_gain(sdrs, rx_samples[0], rx_samples[1], rx_samples[2], rx_samples[3])
    for i in range(sdrs.num_rx_elements):
        rx_samples[i] = arrays_adjusted[i]

    rx_samples = adjust_phase(sdrs, phase, rx_samples[0], rx_samples[1], rx_samples[2], rx_samples[3])
    ...
    data_sum = rx_samples[0] + rx_samples[1] + rx_samples[2] + rx_samples[3] # sum the four subarrays
...
def update_tracker():
    ...
    steer_angle = np.degrees(np.arcsin(max(min(1, (3e8 * np.radians(current_phase)) / (2 * np.pi * signal_freq * elem_spacing)), -1)))
