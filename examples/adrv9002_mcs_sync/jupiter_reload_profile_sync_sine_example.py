...
def reload_profile_and_transmit():
    ...
    tx1_samples = config.amplitude_discrete * (np.cos(2 * np.pi * config.tx_sine_baseband_freq * t1 + phase_shift) + 1j*np.sin(2 * np.pi * config.tx_sine_baseband_freq * t1 + phase_shift))
...
for dev in [sdrs.primary] + sdrs.secondaries:
    for i in range(n_ch):
        line[(dev.uri, i)], = ax.plot(x, [0]*config.num_samps, label=f"{dev.uri} I ch{i}")
...
for list_name in ph_err_list_names:
    line[list_name], = ax2.plot(x_ph_err, [0]*num_samples_ph_err, label=f"{list_name}")
...
def update(frame):
    ...
    # Update plots
    iq0iq1_data = sdrs.rx()
    iq0iq1_data = adjust_phase(sdrs, 10, iq0iq1_data[0], iq0iq1_data[1], iq0iq1_data[2], iq0iq1_data[3])
    iq0iq1_data = adjust_gain(sdrs, iq0iq1_data[0], iq0iq1_data[1], iq0iq1_data[2], iq0iq1_data[3])
