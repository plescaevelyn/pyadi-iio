...
class adrv9002_multi(map_to_multi_devices, adrv9002):
    ...
    def rx_arm(self):
        logger.debug(f"ARM RX transfer path")
    ...
    def rx_enabled_channels(self) -> Union[List[int], List[str]]:
        ...
        for dev in self.secondaries:
            en_channels_dev_secondary = [int(v) + 2 * dev_count for v in dev.rx_enabled_channels]
    ...
    def rx_enabled_channels(self, value: Union[List[int], List[str]]):
        ...
        for chn in value:
            if chn in (0, 1):
                ...
                dev_id = chn // 2
                chn_id = chn % 2
                temp_secondary_en_channels[dev_id-1] = temp_secondary_en_channels[dev_id-1] + [chn_id]
    ...
    def tx_enabled_channels(self) -> Union[List[int], List[str]]:
        ...
        dev_count = 1
        for dev in self.secondaries:
            en_channels_dev_secondary = [int(v) + 2 * dev_count for v in dev.tx_enabled_channels]
            en_channels_dev += en_channels_dev_secondary
            dev_count += 1
